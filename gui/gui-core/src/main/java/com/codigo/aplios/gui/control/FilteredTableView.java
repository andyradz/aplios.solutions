...
public class FilteredTableView<S> extends TableView<S> {
	...
	// Logger.getLogger(FilteredTableView.class);
	...
	public FilteredTableView() {
		...
		final EventHandler<ColumnFilterEvent<?, ?, ?, ?>> columnFilteredEventHandler = event -> {
			...
			if ((col.isFiltered() == true) && (FilteredTableView.this.filteredColumns.contains(col) == false)) {
				...
				// FilteredTableView.logger.debug(String.format("Filter added on column: %s",
				// col.getText()));
				...
				// FilteredTableView.logger.debug(String.format("Filter removed on column: %s",
				// col.getText()));
			}
			...
			if (change.wasRemoved())
				for (final TableColumn<?, ?> col1 : change.getAddedSubList())
					if (col1 instanceof AbstractFilterableTableColumn) {
						...
						// .format("No longer listening for filter changes on column: %s",
						// col1.getText()));
						final AbstractFilterableTableColumn<?, ?, ?, ?> fcol1 = (AbstractFilterableTableColumn<?, ?, ?, ?>) col1;
						fcol1.removeEventHandler(ColumnFilterEvent.FILTER_CHANGED_EVENT, columnFilteredEventHandler);
					}
			if (change.wasAdded())
				for (final TableColumn<?, ?> col2 : change.getAddedSubList())
					if (col2 instanceof AbstractFilterableTableColumn) {
						...
						// .debug(String.format("Now listening for filter changes on column: %s",
						// col2.getText()));
						final AbstractFilterableTableColumn<?, ?, ?, ?> fcol2 = (AbstractFilterableTableColumn<?, ?, ?, ?>) col2;
