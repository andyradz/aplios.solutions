package com.codigo.aplios.domain.model.money;
...
public final class Money implements Comparable<Money>, Serializable {
	 ...
	 *                      decimals in the amount cannot <em>exceed</em> the
	 *                      maximum number of decimals for the given
	 *                      {@link Currency}. It's possible to create a
	 *                      <tt>Money</tt> object in terms of 'thousands of
	 *                      dollars', for instance. Such an amount would have a
	 *                      scale of -3.
	 ...
	 *                        currency. If the collection is empty, then a zero
	 *                        value is returned.
	 * @param currencyIfEmpty is used only when <tt>aMoneys</tt> is empty; that way,
	 *                        this method can return a zero amount in the desired
	 *                        currency.
	 */
	...
	public Money times(final double factor) {
		...
		BigDecimal newAmount = this.mAmount.multiply(asBigDecimal(factor));
		newAmount = newAmount.setScale(getNumDecimalsForCurrency(), this.mRounding);
	...
	public Money div(final double aDivisor) {
		...
		final BigDecimal newAmount = this.mAmount.divide(asBigDecimal(aDivisor), this.mRounding);
		return new Money(newAmount, this.mCurrency, this.mRounding);
	}
	...
	private void validateState() {
		...
		if (this.mAmount.scale() > getNumDecimalsForCurrency())
