...
public class TimeSpan implements Comparable<TimeSpan> {
	...
	public static TimeSpan fromDays(final double days) {
		...
		return TimeSpan.interval(days, TICKSPERDAY);
	}
	
	public static TimeSpan fromHours(final double hours) {
		
		return TimeSpan.interval(hours, TICKSPERHOUR);
	}
	
	public static TimeSpan fromMinutes(final double minutes) {
		
		return TimeSpan.interval(minutes, TICKSPERMINUTE);
	}
	
	public static TimeSpan fromSeconds(final double seconds) {
		
		return TimeSpan.interval(seconds, TICKSPERSECOND);
		
	}
	
	public static TimeSpan fromMilliseconds(final double milliseconds) {
		
		return TimeSpan.interval(milliseconds, TICKSPERMILLISECOND);
	}
	...
	public static final long TICKSPERMINUTE = TimeSpan.TICKSPERSECOND * TimeRelations.SECONDSPERMINUTE.get();
	public static final long TICKSPERHOUR = TimeSpan.TICKSPERMINUTE * TimeRelations.SECONDSPERMINUTE.get();
	public static final long TICKSPERDAY = TimeSpan.TICKSPERHOUR * TimeRelations.HOURSPERDAY.get();
	...
	private static TimeSpan interval(final double value, final long scale) {
	...
	public String toString() {
		...
		str.append(String.format("%03d", this.ticks.get()));
