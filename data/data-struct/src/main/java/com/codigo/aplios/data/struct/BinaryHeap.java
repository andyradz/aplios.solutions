...
public interface BinaryHeap<T extends Comparable<T>>
	...
	public static class BinaryHeapArray<T extends Comparable<T>>
		...
		protected void heapDown(final int index) {
			...
			final T left = (leftIndex != Integer.MIN_VALUE) && (leftIndex < this.size)
					? this.array[leftIndex]
					: null;
			final T right = (rightIndex != Integer.MIN_VALUE) && (rightIndex < this.size)
			...
			if (((this.type == Type.MIN) && (left != null)
					&& (right != null)
					&& (value.compareTo(left) > 0)
					&&
					(value.compareTo(right) > 0)) ||
					((this.type == Type.MAX) && (left != null)
							&& (right != null)
							&& (value.compareTo(left) < 0)
							&&
							(value.compareTo(right) < 0)))
			...
			else if (((this.type == Type.MIN) && (right != null)
					&& (value.compareTo(right) > 0)) ||
					((this.type == Type.MAX) && (right != null)
							&& (value.compareTo(right) < 0))) {
				// Right is greater/lesser than node
				nodeToMove = right;
				nodeToMoveIndex = rightIndex;
			} else if (((this.type == Type.MIN) && (left != null)
					&& (value.compareTo(left) > 0)) ||
					((this.type == Type.MAX) && (left != null)
							&& (value.compareTo(left) < 0))) {
	...
	public static class BinaryHeapTree<T extends Comparable<T>>
		...
		private static int[] getDirections(final int idx) {
			...
			if (directionsSize > 0) {
				...
				while (i >= 0) {
					index = (index - 1) / 2;
					directions[i--] = (index > 0) && ((index % 2) == 0)
							? 1
							: 0; // 0=left, 1=right
				}
			}
		...
		public boolean insert(final T value) {
			...
			return this.add(new Node<>(null, value));
		}
		...
		protected void heapDown(final Node<T> nodeToHeapDown) {
			...
			if ((left != null) && (right != null)
				...
				nodeToMove = right;
					...
					&& (node.value.compareTo(left.value) > 0)) ||
					((this.type == Type.MAX) && (left != null)
							&& (node.value.compareTo(left.value) < 0)))
		...
		protected static class HeapPrinter {
			...
			private static <T extends Comparable<T>> String getString(final Node<T> node, final String prefix,
				...
				if (children != null) {
					for (int i = 0; i < (children.size() - 1); i++)
						builder.append(
								HeapPrinter.getString(children.get(i), prefix + (isTail
										? "    "
										: "│   "), false));
					if (children.size() >= 1)
